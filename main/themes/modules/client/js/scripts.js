...
(function($){
	...
	(function(){
		...
		$('.views_carousel .photoframe.animate_ftb').waypointSynchronise({
			container : '.views_carousel',
			delay : 200,
			offset : 700,
			globalDelay : 400,
			classN : "animate_vertical_finished"
		});
		$('.love_carousel .photoframe.animate_ftb').waypointSynchronise({
			container : '.love_carousel',
			delay : 200,
			offset : 700,
			globalDelay : 400,
			classN : "animate_vertical_finished"
...
$('#shopping_button').on('mouseenter',function(){
  ...
  $(this).css('z-index','189');
	...
	(function(){
		...
		var vc = $('.views_carousel');
		if(vc.length){
			var v = vc.owlCarousel({
		 		itemsCustom : [[1199,4],[992,3],[768,3],[590,2],[300,1]],
		 		autoPlay : false,
		 		slideSpeed : 1000,
		 		autoHeight : true
		 	...
		 	$('.views_prev').on('click',function(){
		 		v.trigger('owl.prev');
		 	});

		 	$('.views_next').on('click',function(){
		 		v.trigger('owl.next');
		 	});
		}

		var lc = $('.love_carousel');
		if(lc.length){
			var l = lc.owlCarousel({
		 		itemsCustom : [[1199,4],[992,3],[768,3],[590,2],[300,1]],
		 		autoPlay : false,
		 		slideSpeed : 1000,
		 		autoHeight : true
		 	...
		 	$('.love_prev').on('click',function(){
		 		l.trigger('owl.prev');
		 	});

		 	$('.love_next').on('click',function(){
		 		l.trigger('owl.next');
		 	});
		}
